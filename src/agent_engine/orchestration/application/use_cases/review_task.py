import logging
from dataclasses import dataclass

from agent_engine.orchestration.domain.ports.agent_session_repository import AgentSessionRepository
from agent_engine.dispatching.application.use_cases.execute_session import ExecuteSession
from agent_engine.dispatching.application.dtos.execute_session_command import ExecuteSessionCommand
from agent_engine.shared.domain.value_objects.task_id import TaskId
from agent_engine.orchestration.domain.enums import SessionStatus
from agent_engine.orchestration.application.dtos.review_task_command import ReviewTaskCommand
from agent_engine.orchestration.application.dtos.review_task_result import ReviewTaskResult

logger = logging.getLogger(__name__)



@dataclass
class ReviewTask:
    """Orchestration 应用层用例：审核任务"""

    session_repository: AgentSessionRepository
    execute_session_use_case: ExecuteSession

    async def execute(self, command: ReviewTaskCommand) -> ReviewTaskResult:
        if command.parent_id is None:
            logger.info(f"任务 {command.task_id} 无父任务，等待人工审核")
            return ReviewTaskResult(status="pending_manual_review")

        session = await self.session_repository.find_by_task_id(
            TaskId.reconstitute(command.parent_id)
        )

        if session is None:
            logger.warning(f"未找到 parent_id={command.parent_id} 对应的 Session")
            return ReviewTaskResult(
                status="failed",
                fault=f"Parent session not found for task_id={command.parent_id}",
            )

        user_prompt = f"审核任务：{command.task_id}"

        session.add_user_message(content=user_prompt)
        session.status = SessionStatus.PROCESSING
        await self.session_repository.save(session)
        
        exec_command = ExecuteSessionCommand(
            system_prompt="",
            user_prompt=user_prompt,
            session_id=str(session.id),
            project_id=str(session.project_id),
            context_payload={},
        )

        exec_result = await self.execute_session_use_case.execute(exec_command)

        session.add_agent_message(content=exec_result.output or "")
        session.status = SessionStatus.IDLE
        await self.session_repository.save(session)

        return ReviewTaskResult(
            session_id=str(session.id),
            status=exec_result.status.value,
            output=exec_result.output,
            fault=exec_result.fault,
        )
