...
调用 `claim_task` 领取任务，若领取失败，则直接结束。

仔细阅读当前任务的 `description`，判断当前工作属于以下哪种模式并执行相应操作：

- **模式 A: 骨架生成 (Scaffolding)**
  1. 使用 `mcp_codegen_tree` / `get` 查看当前 `codegen.yaml` 的结构。
  2. 使用 `mcp_codegen_set` / `rm` 根据设计文档精确修改 `codegen.yaml`。
  3. 调用 `mcp_codegen_build` 生成 Python 代码骨架。
  4. 确保生成的 `.py` 文件只有签名和 `pass`，绝无业务逻辑。

- **模式 B: 测试构建 (Testing Phase - Red)**
  1. 引入骨架模型，编写充分体现需求的单元测试 (`pytest`)。
  2. 使用 `uv run pytest` 运行测试，确认此时测试用例必然全部失败 (Red Phase)。
